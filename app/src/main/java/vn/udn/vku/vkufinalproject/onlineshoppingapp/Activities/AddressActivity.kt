...
import vn.udn.vku.vkufinalproject.onlineshoppingapp.Adapter.AddressAdapter
import vn.udn.vku.vkufinalproject.onlineshoppingapp.Models.AddressModel
...
class AddressActivity : AppCompatActivity(), AddressAdapter.SelectedAddress {
    ...
    private fun getDataTransfer() {
        ...
        val totalPrice: Int
        val cc = intent.extras
        if (intent.extras != null) {
            Log.w("Intentt", intent.getStringExtra("productName").toString())
            Log.w("Intentt", intent.toString())
